...
def stage1_segment(model, img_bgr):
    ...
    if contours:
        ...
        # Build segmentation visual
        seg_visual = img_bgr.copy()
        cv2.drawContours(seg_visual, [largest], -1, (0, 255, 0), 2)
        cv2.rectangle(seg_visual, (x, y), (x+w, y+h), (255, 0, 0), 2)
    else:
        cropped_img = masked_img
        seg_visual = img_bgr.copy()
        
    return seg_visual, cropped_img
...
def evaluate_four_rotations(model, img_pil):
    ...
    for angle, candidate_img in candidates.items():
        ...
        p0 = float(probs[0].item()) # class0 is upright (assuming order 0, 90, 180, 270)
...
def stage5_kie(kie_model, processor, norm_img, boxes, texts):
    ...
    for i in range(len(boxes)):
        if i in valid_indices:
            ...
            final_preds.append(other_id)
            final_scores.append(0.0)
            
    return final_preds, final_scores, extracted
...
def create_panel_title(img_w, title, bg_color=(40, 40, 40), text_color=(255, 255, 255), height=40):
    panel = np.full((height, img_w, 3), bg_color, dtype=np.uint8)
    ...
    tx = 15
    ty = (height + th) // 2
    cv2.putText(panel, title, (tx, ty), font, font_scale, text_color, thickness, cv2.LINE_AA)
    return panel

def add_title_to_image(img_bgr, title):
    h, w = img_bgr.shape[:2]
    title_bar = create_panel_title(w, title)
    return np.vstack([title_bar, img_bgr])
...
def build_panel4_detection(norm_img, boxes):
    img_bgr = cv2.cvtColor(norm_img, cv2.COLOR_GRAY2BGR)
    for box in boxes:
        x1, y1, x2, y2 = map(int, box[:4])
...
def draw_kie_boxes(img_bgr, boxes, pred_ids, scores):
    ...
    if VIET_FONT:
        try:
            font = ImageFont.truetype(VIET_FONT.get_file(), 14)
        except:
            font = ImageFont.load_default()
    else:
        font = ImageFont.load_default()
        
    for box, pred_id, score in zip(boxes, pred_ids, scores):
        label = ID2LABEL.get(pred_id, "OTHER")
        ...
        lx2, ly2 = x1 + tw + 4, max(0, y1)
        draw.rectangle([lx1, ly1, lx2, ly2], fill=(r, g, b, 240))
        draw.text((lx1 + 2, ly1 + 1), text, font=font, fill=(255, 255, 255, 255))
        
    return cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)

def draw_table(extracted, width=600, height=800):
    ...
    draw.text((20, 20), "EXTRACTED INFORMATION TABLE", font=font_title, fill=(0, 0, 0))
    draw.line([(20, 60), (width - 20, 60)], fill=(200, 200, 200), width=2)
    
    y = 80
    line_spacing = 35
    
    draw.text((20, y), "FIELD", font=font_bold, fill=(50, 50, 50))
    draw.text((220, y), "VALUE", font=font_bold, fill=(50, 50, 50))
    y += line_spacing
    ...
    for label, texts in extracted.items():
        ...
        draw.text((20, y), label, font=font_regular, fill=(0, 0, 0))
        draw.text((220, y), val, font=font_regular, fill=(0, 0, 0))
        y += line_spacing
...
def parse_args():
    ...
    parser.add_argument("--image", type=str, required=True, help="Path to raw receipt image")
    parser.add_argument("--output", type=str, default="demo_result.png", help="Output composite image path")
...
def main():
    ...
    print("Running Stage 1: Segmentation...")
    seg_visual, cropped_img = stage1_segment(models['seg'], raw_img)
    
    print("Running Stage 2: Normalization...")
    norm_gray = stage2_normalize(models['rot'], cropped_img)
    
    print("Running Stage 3: Text Detection...")
    boxes = stage3_detect_text(models['det'], norm_gray)
    
    print("Running Stage 4: OCR...")
    texts = stage4_ocr(models['ocr'], norm_gray, boxes)
    
    print("Running Stage 5: KIE...")
    pred_ids, scores, extracted = stage5_kie(models['kie_model'], models['kie_processor'], norm_gray, boxes, texts)
    
    print("Composing final visualization...")
    p1 = add_title_to_image(raw_img, "1. Raw Input")
    p2 = add_title_to_image(cropped_img, "2. Segmented & Cropped")
    p3 = add_title_to_image(cv2.cvtColor(norm_gray, cv2.COLOR_GRAY2BGR), "3. Normalized")
    
    h_r1 = max(p1.shape[0], p2.shape[0], p3.shape[0])
    p1 = resize_to_height(p1, h_r1)
    p2 = resize_to_height(p2, h_r1)
    p3 = resize_to_height(p3, h_r1)
    
    row1 = np.hstack([p1, p2, p3])
    
    p4 = add_title_to_image(build_panel4_detection(norm_gray, boxes), "4. Text Detection")
    
    img_kie = draw_kie_boxes(cv2.cvtColor(norm_gray, cv2.COLOR_GRAY2BGR), boxes, pred_ids, scores)
    
    table_height = max(img_kie.shape[0], 400)
    table_img = draw_table(extracted, width=600, height=table_height)
    
    p5_combined = np.hstack([img_kie, table_img])
    p5 = add_title_to_image(p5_combined, "5. Key Information Extraction")
    
    h_r2 = max(p4.shape[0], p5.shape[0])
    p4 = resize_to_height(p4, h_r2)
    p5 = resize_to_height(p5, h_r2)
    
    row2 = np.hstack([p4, p5])
    ...
    if w1 < w_max:
        pad = np.full((row1.shape[0], w_max - w1, 3), 255, dtype=np.uint8)
        row1 = np.hstack([row1, pad])
    elif w2 < w_max:
        pad = np.full((row2.shape[0], w_max - w2, 3), 255, dtype=np.uint8)
